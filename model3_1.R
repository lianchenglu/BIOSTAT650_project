...
rm.m_3 =  lm(
  BMI ~ SleepHrsNight +Age + Gender + Race1  + Poverty + TotChol+ BPDiaAve + BPSysAve + AlcoholYear+ Smoke100 + UrineFlow1 + DaysMentHlthBad +DaysPhysHlthBad+HealthGen+PhysActive, rm3.df3
)
...
p34.log = ggplot() + geom_point(aes(y = rstudent(m_3.log), x = m_3.log$fitted.values )) + labs(x = "Predicted Value", y = "Jacknife Residuals") +geom_hline(yintercept = c(-2,2))
grid.arrange(p33.log,p34.log, nrow=2)


p33 = ggplot(m_3, aes(sample = rstudent(m_3))) + geom_qq() + stat_qq_line() +labs(title="Q-Q plot")
p34 = ggplot() + geom_point(aes(y = rstudent(m_3), x = m_3$fitted.values )) + labs(x = "Predicted Value", y = "Jacknife Residuals") +geom_hline(yintercept = c(-2,2))
